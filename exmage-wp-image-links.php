...
if (!class_exists('EXMAGE_WP_IMAGE_LINKS')) {
	class EXMAGE_WP_IMAGE_LINKS
	{
		...
		{
			$_brandy_external_url = get_post_meta($attachment_id, '_brandy_external_url', true);
			if ($_brandy_external_url && !get_post_meta($attachment_id, '_exmage_imported', true)) {
				update_post_meta($duplicated_attachment_id, '_brandy_external_url', $_brandy_external_url);
			}
		}
...
?>
		...
		{
			if (get_post_meta($attachment_id, '_brandy_external_url', true) && !get_post_meta($attachment_id, '_exmage_imported', true)) {
				return false;
			}
			...
			<div>
				...
				<div class="exmage-use-url-container">
					...
					<div class="exmage-use-url-input-container">
						...
						?>
							<input type="search" id="exmage-use-url-input" class="exmage-use-url-input" placeholder="<?php esc_attr_e('Paste an external image URL here or press Enter after you type to process', 'exmage-wp-image-links') ?>">
						<?php
						}
						?>
		...
		{
			$_brandy_external_url = '';
			if (!get_post_meta($response['id'], '_exmage_imported', true)) {
				$_brandy_external_url = get_post_meta($response['id'], '_brandy_external_url', true);
				//				if ( $_brandy_external_url ) {
				//					$response['can']['save'] = false;
				//				}
			}
			$response['_brandy_external_url'] = $_brandy_external_url;
		...
		?>
			<script type="text/html" id="tmpl-exmage-attachment">
				...
				<# if ( data.buttons.check ) { #>
					...
					<# } #>
						<# if ( data.hasOwnProperty('_brandy_external_url')&&data._brandy_external_url ) { #>
							<span class="exmage-is-external-link" title="<?php esc_html_e('This is an external media file', 'exmage-wp-image-links'); ?>">
								<svg width="16" height="16" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M3.05455 31.7829V38.9455H38.9454V16.9909H3.05455V31.3546L9.73843 17.0209L24.6373 23.9684L17.7894 38.6539L3.05455 31.7829ZM3.05455 13.9364H38.9454V3.05455H3.05455V13.9364ZM0 0H42V42H0V0ZM6.9364 30.2668L16.2915 34.6291L20.5914 25.408L11.2363 21.0457L6.9364 30.2668Z" fill="black" />
								</svg>
							</span>
							<# } #>
			...
			?>
				<div class="exmage-external-url-container">
					<div class="exmage-external-url-content">
						...
						$external_link    = get_post_meta($post->ID, '_brandy_external_url', true);
		...
		private static function html_for_convertable_external_image($attachment_id)
		{
			...
			<p class="exmage-action-buttons-container">
				<span class="button exmage-convert-external-button" data-attachment_id="<?php echo esc_attr($attachment_id) ?>" title="<?php esc_attr_e('Change this image to use external link. The existing image file stored on your server will be deleted.', 'exmage-wp-image-links') ?>">
		...
		{
		?>
			<a target="_blank" href="<?php echo esc_url($external_link) ?>"><span class="exmage-external-url"><?php echo esc_html($external_link) ?></span><span class="dashicons dashicons-external"></span>
			</a>
			<p class="exmage-action-buttons-container">
				<span class="button exmage-migrate-button" data-attachment_id="<?php echo esc_attr($attachment_id) ?>" title="<?php esc_attr_e('Save this image to your WordPress server like normal images so that it will be editable', 'exmage-wp-image-links') ?>">
...
<?php
		...
		public function convert_external_image()
		{
			...
			if ($attachment_id) {
				...
				if ($attachment && $attachment->post_type === 'attachment') {
					$url              = get_post_meta($attachment_id, '_brandy_external_url', true);
		...
		{
			if (!get_post_meta($attachment_id, '_brandy_external_url', true)) {
				update_post_meta($attachment_id, '_brandy_external_url', $url);
			}
		...
		public function wp_get_attachment_url($url, $id)
		{
			...
			if (!get_post_meta($id, '_brandy_external_url', true)) {
				return $url;
			}
			$post = get_post($id);
