...
!(function (jq) {
	...
	var me = {
		...
		setupItemDeletionListener: function () {
			jq(document).ajaxComplete(function (event, xhr, settings) {
				if (
					settings &&
					settings.data &&
					settings.data.indexOf("action=delete-post") > -1
				) {
					me.getFolderTree();
				}
			});
		...
		setupMainPanelUi: function () {
			...
			jq("#header_action_add_folder button").on("click", function () {
		...
		addControlPanelToUpload: function () {
			...
			jq(wpMediaSectionId).wrapAll(
				'<div class="wrap-media-plus-control-panel"></div>'
			);
			var sidebar_html = wp.element.renderToString(me.getControlPanel());
			sidebar_html += wp.element.renderToString(me.getSplitter());
			jq(wpMediaSectionId).before(sidebar_html).css("flex", "1");
		},

		getCard: function (id, className, countId, iconPath, label, count) {
			var iconElement = wp.element.createElement("img", {
				class: "header-card-icon " + className,
				src: iconPath,
			});
			var labelElement = wp.element.createElement(
				"span",
				{ class: className + " card-label" },
				label
			);
			var countElement = wp.element.createElement(
				"span",
				{ class: className + " card-count", id: countId },
				count
			);
		...
		getButton: function (id, label, iconPath) {
			var iconElement = wp.element.createElement(
				"svg",
				{
			...
			return wp.element.createElement(
				...
				wp.element.createElement(
					"button",
					{
						class: "cursor-pointer d-flex center-center ml-auto gap-2",
						style: "color: var(--acc-button); border: none",
					},
					iconElement,
					wp.element.createElement(
						"span",
		...
		getHeader: function (title, cards, buttons) {
			var titleSection = wp.element.createElement(
				"div",
				{
					class: "header-title w-full d-flex items-center justify-betweeen",
				},
				wp.element.createElement(
					"span",
					{ class: "header-label" },
		...
		getControlPanel: function (showTitle = true, showButtons = true) {
			...
			var header = me.getHeader(
				showTitle ? accmo_i18n["folders"] : null,
				cards,
				buttons
			);
			var folderTree = wp.element.createElement("div", {
				id: "acclectic-jstree",
				class: "pb-3 mb-3",
			});
		...
		getLoader() {
			...
			return wp.element.createElement("div", {
				id: "loader",
				class: "absolute inset-0 bg-black54 d-flex center-center",
		...
		setupBrandyAssetsUI: function (assets) {
			// console.log("Brandy assets: ", assets);
			me.assets = assets;

			var brandyAssetCard = function (asset) {
				var image = wp.element.createElement("img", {
					class: "p-2 object-contain object-center w-full h-full",
					style: "min-width: 0px; max-width: 90%; max-height: 90%; filter: drop-shadow(rgba(0, 0, 0, 0.2) 0.5px 0.5px 1px);",
				...
				var media = wp.element.createElement(
					"div",
					{
						class: "d-flex center-center relative",
						style: "background: rgb(247, 247, 247); width: 100%; aspect-ratio: 1 / 0.8;",
					},
					image
				);

				var text = wp.element.createElement(
					"div",
					{
						class: "border-t border-light-gray bg-black26",
						style: "text-align: left; padding: 0.25rem 0.6rem; font-size: 0.65rem; line-height: 1.5; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; ",
					},
					asset.name
				);
				...
				return wp.element.createElement(
					...
					{
						type: "button",
						class: "brandy-asset-card p-0 cursor-pointer w-full rounded-sm border cusor-pointer parent gray-on-hover relative overflow-hidden",
			...
			var brandyAssetGrid = wp.element.createElement(
				"div",
				{
					class: "d-grid w-full mb-2 gap-2",
					style: "grid-template-columns: 1fr 1fr",
				},
				...assets.map(brandyAssetCard)
			);

			var brandyAssetsHeading = wp.element.createElement(
				"div",
				{
					class: "text-xl mb-4",
				},
			...
			jq("#acclectic-jstree").after(
				wp.element.renderToString(brandyAssets)
			);

			jq(".brandy-asset-card").on("click", function (a) {
				var assetButton = jq(this);
				var image = assetButton.find("img");
				var loader = wp.element.renderToString(me.getLoader());
				image.after(loader);
				...
				jQuery.ajax({
					...
					data: {
						...
						_exmage_ajax_nonce:
							exmage_admin_params._exmage_ajax_nonce,
					...
					success(response) {
						...
						if (response.status === "success") {
							var newItems = response.details.map(({ id }) => id);
							console.log("Asset upload: ", response, newItems);
							me.assignMediaFolder(newItems);
							assetButton.find("#loader").remove();
							// me.reloadGridItems(me.getSelectedFolder());
							// me.reloadList(me.getSelectedFolder());
							...
							// $message.html(
							// 	'<p class="exmage-message-queue"><span class="exmage-use-url-message-content">' +
							// 		response.message +
							// 		"</span></p>"
							// );
						} else {
							// $message.html(
							// 	'<p class="exmage-message-error"><span class="exmage-use-url-message-content">' +
							// 		response.message +
							// 		"</span></p>"
							// );
						}
					...
					error() {
						console.error("Asset upload failed: ", response);
						// $message.html(
						// 	'<p class="exmage-message-error"><span class="exmage-use-url-message-content">An error occurs.</span></p>'
						// );
					},
					complete() {
		...
		getSplitter: function () {
			var handle = wp.element.createElement("img", {
				class: "splitter-handle",
				src: config.assetsPath + "resizer_handle.png",
			});
			return wp.element.createElement(
				"div",
				{ class: "splitter" },
		...
		setupModalUi: function (browser) {
			...
			frameMenu[0].views.add(
				new wp.media.View({
					el: wp.element.renderToString(
						me.getControlPanel(false, false)
					),
				})
			);
		...
		setupGridDraggables: function () {
			if (wp.media !== undefined && wp.media.view !== undefined) {
				...
				wp.media.view.Attachment.Library =
					wp.media.view.Attachment.Library.extend({
						initialize: function () {
							originalLibrary.prototype.initialize.apply(
								this,
								arguments
							);
		...
		handleDndStop: function (data) {
			...
			if (targetNode == null || !targetNode) {
				...
				if (
					jq(data.event.target).attr("class") == undefined ||
					!jq(data.event.target)
						.attr("class")
						.includes("media_organizer_unassigned")
				) {
					console.log(
						"Drop target is not unassigned folder either. Returning."
					);
					return;
				}

				console.log(
					"Unassigning " + data.data.selectedItems.length + " items."
				);
			...
			if (
				data.data.selectedItems === undefined ||
				data.data.selectedItems.length <= 0
			) {
		...
		getSelectedItemsInGrid: function (event) {
			...
			var selectedItems = [];
			var draggables = jq(
				".attachments .selected:not(.selection,:hidden)"
			);

			// Get the dragged item by data-id.
			if (
				(config.mediaLibraryMode == "grid" &&
					inSelectMode &&
					draggables.length) ||
		...
		getSelectedItemsInList: function (event) {
			...
			var selectedItems = [];
			var draggables = jq(
				".wp-list-table input[name='media[]']:input:checked"
			);
		...
		getDragText: function (num) {
			var contentString =
				parseInt(num) > 1
					? accmo_i18n["movingPluralItems"]
					: accmo_i18n["movingSingularItem"];
			tooltip = wp.element.createElement(
				DragTooltip,
				{ contents: contentString, n: num },
				null
			);
		...
		setSelectedFolderFromUrl: function () {
			...
			me.setSelectedFolder(
				selectedFolderFromUrl != null
					? selectedFolderFromUrl
					: ALL_FOLDER_ID
			);
		},
		...
		/**
		 * Gets the Brandy assets for currently selected brand.
		 */
		getBrandyAssets: function () {
			var request = {
				action: "acclectic_get_brandy_assets",
				nonce: me.nonce,
			};

			var handler = function (response) {
				if (!response.success) {
					console.log("Error on getBrandyAssets.");
					return;
				}

				me.setupBrandyAssetsUI(response.data);
		...
		assignMediaFolder: function (selectedItems, targetNode) {
			...
			if (["all", "unassigned"].includes(targetNodeId)) {
				// Selection normally triggers refresh, but not here because we select the same folder.
				me.reloadGridItems(me.getSelectedFolder());
				me.reloadList(me.getSelectedFolder());
			...
			var handler = function (response) {
				...
				if (response.data.assigned) {
					console.log(
						"Number of items assigned: " + response.data.assigned
					);
					// TODO: Efficiency improvement: remove items instead of reloading the page.
		...
		reloadList: function (selectedNode) {
			...
			jq("#media-folder-filter-list").append(
				'<option value="' +
					selectedNode +
					'">' +
					selectedNode +
		...
		reloadGridFilters: function () {
			jq(".attachments-browser").each(function () {
				jq(this)
					.data("backboneView")
					.toolbar.get(MEDIA_LIBRARY_FOLDER_FILTER_KEY)
					.createFilters();
			});
		},
		...
		reloadGridItems: function (selectedNode) {
			...
			jq(".attachments-browser").each(function () {
				...
				var selectElement = jq(this)
					.data("backboneView")
					.toolbar.get(MEDIA_LIBRARY_FOLDER_FILTER_KEY).$el;
				selectElement.empty();
			...
			wp.media.view.AttachmentsBrowser =
				wp.media.view.AttachmentsBrowser.extend({
					createToolbar: function () {
						console.log(
							"Extending AttachmentsBrowser and creating toolbar."
						);
						...
						this.toolbar.set(
							MEDIA_LIBRARY_FOLDER_FILTER_KEY,
							filter
						);
						filter.initialize();
					},
			...
			items: function (o, cb) {
				return {
					create: {
						...
						icon:
							me.jstreeUrl +
							"/themes/acclectic/folder_add_dark.svg",
						label: accmo_i18n["newFolder"],
					...
					rename: {
						...
						_disabled: false,
						icon:
							me.jstreeUrl + "/themes/acclectic/folder_edit.svg",
					...
					remove: {
						...
						icon:
							me.jstreeUrl +
							"/themes/acclectic/folder_delete.svg",
						label: accmo_i18n["deleteFolder"],
						action: function (data) {
							var folderNameToDelete = tree
								.get()
								.get_node(data.reference).text;

							acclecticDialog
								.getModal({
									title: accmo_i18n[
										"deleteFolderTitle"
									].formatUnicorn({ f: folderNameToDelete }),
									description:
										accmo_i18n["deleteFolderDescription"],
									buttons: [
										{
											label: accmo_i18n["ok"],
											handler: function () {
												tree.deleteFolder(data);
											},
										},
		...
		setupNewTree: function (treeData) {
			jq("#acclectic-jstree").jstree({
				...
				types: {
					default: {
						icon:
							me.jstreeUrl + "/themes/acclectic/folder-36px.png",
					},
				},
				core: {
					themes: {
			...
			jq("#acclectic-jstree").on(
				"create_node.jstree",
				tree.createNodeHandler
			);
			jq("#acclectic-jstree").on(
				"rename_node.jstree",
				tree.updateNodeHandler
			);
			jq("#acclectic-jstree").on(
				"move_node.jstree",
				tree.updateNodeHandler
			...
			jq("#acclectic-jstree").on(
				"delete_node.jstree",
				tree.deleteNodeHandler
			);
			jq("#acclectic-jstree").on(
				"select_node.jstree",
				tree.selectNodeHandler
			);
		},
