...
class ControlPanel
{
    ...
    {
        wp_enqueue_style(
            'media-organizer-utils',
            ACCLECTIC_CSS_URL . 'media-organizer-utils.css',
            array(),
    ...
    public function getBrandyAssets()
    {
        $args = array(
            'headers' => array(
                'Authorization' => "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Indha3lqMDdAZ21haWwuY29tIiwidWlkIjoiNjRkNjE3ODNiMDJhMzVhODczOWVhNDZkIiwiZmlyc3RfbmFtZSI6IldhbHRlciIsImlhdCI6MTY5NTIyNjc1OSwiZXhwIjoxNzI2NzYyNzU5fQ.AnOLirQ061HVjegeGX_EejXSFfD4d4pQPLtt0nhFrp8"
            )
        );
    ...
    public function addFilterByFolderIdForGrid($query)
    {
        ...
        if (
            $show_folder_id != ACCLECTIC_PARAM_ALL_FOLDER_ID
    ...
    private function getItemsInFolderFromDb($show_folder_id)
    {
        ...
        return $wpdb->get_col($wpdb->prepare(
            'SELECT * FROM `%1$s` WHERE `parent_id` LIKE "%2$s"',
            $this->items_table,
            $show_folder_id
        ));
    }
    ...
    private function deleteFolderWithChildren($folder_id)
    {
        ...
        $child_folders = $wpdb->get_col(
            $wpdb->prepare(
                'SELECT `node_id` FROM `%1$s` WHERE `parent_id` LIKE "%2$s"',
                $this->folders_table,
                $folder_id
            )
        );
